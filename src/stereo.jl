...
"""
function stereocenter_from_smiles(g::SimpleGraph{T}, edge_rank, v_stereo) where T
    centers = Stereocenter{T}()
    for i in vertices(g)
        degree(g, i) in (3, 4) || continue
        ...
        nbrs = neighbors(g, i)
        o = sortperm([edge_rank[u_edge(T, i, nbr)] for nbr in nbrs])
...
stereocenter_from_smiles(mol::SimpleMolGraph) = stereocenter_from_smiles(
    mol.graph, mol.edge_rank, [get_prop(mol, i, :stereo) for i in vertices(mol)])
