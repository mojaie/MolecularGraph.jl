...
@testset "chain" begin
    ...
    @test nitro[1, 2].order == 3

    vinylcl = smilestomol("C=CCl")
    @test vinylcl[1, 2].order == 2
    @test vinylcl[3].symbol === :Cl
end

@testset "branch" begin
    tBuOH = smilestomol("CC(C)(C)O")
    @test tBuOH[5].symbol === :O
    @test degree(tBuOH, 2) == 4

    TFA = smilestomol("FC(F)(F)C(=O)O")
    @test TFA[5, 6].order == 2
    @test degree(TFA, 2) == 4
...
@testset "ring" begin
    ...
    @test cychexe2[6, 1].order == 2

    cychexe3 = smilestomol("C1CCCCC=1")
    @test cychexe3[6, 1].order == 2

    cychexe4 = smilestomol("C=1CCCCC=1")
    @test cychexe4[6, 1].order == 2

    bicyclohexyl = smilestomol("C1CCCCC1C1CCCCC1")
    @test bicyclohexyl[6, 1].order == 1
    @test bicyclohexyl[12, 7].order == 1

    bicyclohexyl2 = smilestomol("C1CCCCC1C2CCCCC2")
    @test bicyclohexyl2[6, 1].order == 1
    @test bicyclohexyl2[12, 7].order == 1

    naphthalene = smilestomol("C=1C=CC=C2C1C=CC=C2")
    @test naphthalene[6, 1].order == 2
    @test naphthalene[10, 5].order == 1

    naphthalene2 = smilestomol("C1=CC=C2C(=C1)C=CC=C2")
    @test naphthalene2[6, 1].order == 1
    @test naphthalene2[10, 4].order == 1
...
@testset "component" begin
    ...
    @test NaCl[1].symbol === :Na
    @test NaCl[1].charge == 1
    @test NaCl[2].symbol === :Cl
    @test NaCl[2].charge == -1

    ferrous = smilestomol("[Fe++]")
    @test ferrous[1].symbol === :Fe
    @test ferrous[1].charge == 2

    ferrous2 = smilestomol("[Fe+2]")
    @test ferrous2[1].symbol === :Fe
    @test ferrous2[1].charge == 2

    CuSO4 = smilestomol("[Cu+2].[O-]S(=O)(=O)[O-]")
    @test CuSO4[1].charge == 2
    @test CuSO4[6].charge == -1
...
@testset "stereo" begin
    ...
    @test LAla[2].stereo === :anticlockwise
    DAla = smilestomol("N[C@@H](C)C(=O)O")
    @test degree(DAla, 2) == 4
    @test DAla[2].stereo === :clockwise
end

@testset "aromatic" begin
    benzene = smilestomol("c1ccccc1")
    @test benzene[1].isaromatic
    @test benzene[1, 6].order == 1
    pyrrole = smilestomol("[nH]1cccc1")
    @test pyrrole[1].isaromatic
