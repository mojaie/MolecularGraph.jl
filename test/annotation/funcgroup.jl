...
@testset "annotation.funcgroup" begin
...
@testset "fgrouprecord" begin
    ...
    coumarin.annotation[:FunctionalGroup] = FunctionalGroup()
    gr = coumarin.annotation[:FunctionalGroup].nodeset
    gr[:Carbonyl] = fgroupquery(coumarin, "[#6]=[OD1]")
    res1 = fgrouprecord(
        coumarin, Dict("have" => ["Carbonyl"], "query" => "[#6](=O)[#8]"))
...
@testset "functionalgroup" begin
    ...
    functionalgroup!(paclitaxel)
    res = paclitaxel.annotation[:FunctionalGroup].nodeset
